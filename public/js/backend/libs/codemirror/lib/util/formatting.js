...
//        console.log(cmInstance.getLine(i));
      cmInstance.indentLine(i, "smart");
    }
  });
});
  ...
  autoFormatLineBreaks: function (text, startPos, endPos) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (mToProcess != null && mToProcess.length > 3) { // The line starts with whitespaces and ends with whitespaces
        ...
        var cdata_start = lines[i].indexOf('[CDATA['),
            cdata_stop  = lines[i].indexOf(']]>');
        if (cdata_start > 0) {
            var value = lines[i].substr(cdata_start+7, cdata_stop - cdata_start - 7 ).split("\n").join('');
            lines[i] = lines[i].replace(lines[i].substr(cdata_start+7, cdata_stop - cdata_start - 7 ), value);
        }
